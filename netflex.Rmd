...
output$plotPref <- renderPlot({
    ...
    if (input$profile != "All") {  
       freq_data <- as.data.frame(freqfunc(dat[which(dat$Profile==input$profile),]$Title, 5))
    } else {
               validate(
    need(input$profile != "All", "Please choose a profile."))
    }
