...
renderPlot({
  wordcloud(words = as.character(genres$Genre), freq = genre$Count, min.freq = 1, max.words=25, random.order=FALSE, rot.per=0.35,
...
renderLeaflet({
# Color palette
newdfx <- newdf[newdf$Account==as.character(input$profile),]
...
reactive_df <- reactive({
          ...
          if (input$leaveVal){
            return(genres[-26,-c(3,25)])
          } else {
            return(genres[,-c(3,25)])}
})
